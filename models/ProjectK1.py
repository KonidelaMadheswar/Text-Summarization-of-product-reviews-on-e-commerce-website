...
class Flipkart:
    ...
    @staticmethod
    def process_flipkart(product_name):
        ...
        data = pd.read_csv('./models/flipkart_reviews.csv')
        ...
        preprocessed_reviews = [Flipkart.preprocess_text(review, nlp) for review in product_reviews]

        # Process reviews in parallel
        with ThreadPoolExecutor(max_workers=16) as executor:
            results = list(executor.map(Flipkart.process_review, preprocessed_reviews, [tokenizer]*len(preprocessed_reviews), [summarization_model]*len(preprocessed_reviews), [sentiment_analysis_pipeline]*len(preprocessed_reviews)))
